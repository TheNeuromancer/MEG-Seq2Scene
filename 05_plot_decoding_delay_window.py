...
for label in all_labels:
    ...
    conds = natsorted([x for x in all_AUC[-1].keys()])
    for train_cond in conds:
        for test_cond in conds:
            for i_sub in range(len(all_subs)-n_lab_subs, len(all_subs)):
                try:
...
print(f"\nDoing {all_labels}")
out_fn = f"{out_dir}/delay_all_conds_{len(all_AUC)}ave"
...
# for cond_str1, cond_str2 in zip(["Colour1", "Colour2"], ["Shape1", "Shape2"], ["Shape1+Colour1", "Shape2+Colour2"]):
# for cond_str1, cond_str2 in zip(["Colour1", "Shape1", "Shape1+Colour1"], ["Colour2", "Shape2", "Shape2+Colour2"]):
box_pairs = []
    # try:
    ...
    #     set_trace()
make_sns_barplot(df, x='Tested on', y='AUC', hue='Trained on', box_pairs=box_pairs, out_fn=f'{out_fn}_AUC.png', hline=0.5, ymin=0.45, rotate_ticks=True, tight=True, ncol=4)
    # except:
