...
parser.add_argument('--regress', action='store_true',  default=False, help='regression single channel results')
...
decoding_dir = f"Decoding_single_ch_v{args.version}" if not args.regress else f"Regression_decoding_single_ch_v{args.version}"
...
if args.regress:
    all_fns = natsorted(glob(in_dir + '/*all_windows_R.p'))
else:
        ...
        for split_query in ["match", "nonmatch", "flash", "noflash", False]:
            for gen_cond in [None, "localizer", "obj", "scenes"]: # , "one_object", "two_objects", 
                ...
                for fn in all_fns:
                    ...
                    if args.regress:
                        all_data.append(pickle.load(open(fn, "rb")))
                    else:
                        all_data.append(np.load(fn))
                ...
                out_fn = f"{out_dir}/{label}_trained_on_{train_cond}{gen_str}{split_query_str}_{len(all_data)}ave"

                if args.save_loc:
                    AUC_dict = {s: auc for s, auc in zip(all_subs, all_data)}
                ...
                if args.regress:
                    for win in all_data[0].keys():
                        all_Rs = [sub_dat[win] for sub_dat in all_data]
                        vmin, vcenter, vmax = -.05, 0, .05
                        epo_info = pickle.load(open(fn.replace('all_windows_R.p', 'epo_info.p'), "rb")) ## get epochs info

                        title = f"{op.basename(out_fn).split(str(len(all_Rs))+'ave')[0]}".replace("_", " ")
                        plot_single_ch_perf(np.mean(all_Rs, 0), epo_info, f"{out_fn}_{win}_mean_R.png", title=title, vmin=vmin, vcenter=vcenter, vmax=vmax)
                        # plot_single_ch_perf(np.median(all_Rs, 0), epo_info, f"{out_fn}_{win}_median_R.png", title=title, vmin=-.1, vcenter=vcenter, vmax=.1)
                    

                else: # classsical single ch decoding plotting
                    # average per subject, then accross subjects
                    all_AUC = np.array(all_data)
                    AUC_mean = np.mean(all_data, 0)
                    AUC_med = np.median(all_data, 0)
                    AUC_std = np.std(all_data, 0)

                    epo_info = pickle.load(open(fn.replace('AUC_diag.npy', 'epo_info.p'), "rb")) ## get epochs info
