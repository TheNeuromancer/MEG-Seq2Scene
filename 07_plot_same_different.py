...
parser.add_argument('-k', '--kind',  default='corr', help='corr of decod')
...
_dir = f"Correlation_v{args.version}" if args.kind=='corr' else f"Same_Diff_Decod_v{args.version}"
...
for mirror in (True, False):
    for window in (True, False):
        for reducdim in (True, False):
            ...
            if not data:
                print(f"did not find any data for mirror={mirror} and window={window}, and reducdim={reducdim}, moving on")
                continue
            print(f"Doing  mirror={mirror} and window={window}, and reducdim={reducdim}, found {len(all_subs)} subjects")
            ...
            if args.kind == "corr":
                ...
                for ch in range(data.shape[1]): # test each channel
                    ...
                    pvals.append(pval)
                corrected_pvals = fdr_correction(np.array(pvals), alpha=0.8)[1]
                ...
                print(f"Decoding perf for: AUC = {np.mean(data):.3f}")

            mirror_str = "_mirror" if mirror else ""
            window_str = "_window" if window else ""
            # print(f"\nDoing {mirror_str} and ")
            out_fn = f"{out_dir}/{len(data)}ave{mirror_str}{window_str}"
            n_subs = len(all_subs)
